function [F,G] = edRVFL_FIS_defuzzy_test(test_x,NumFuzzyRule,Alpha,center)
%% Training starts
std = 1;
tic
Omega=zeros(size(test_x,1),NumFuzzyRule);

F = zeros(size(test_x,1), NumFuzzyRule); %Fuzzy Layer
...
% end
%%
for j = 1:size(test_x,1)   %calculating fuzzy membership "mew" - equation 10
    MF = exp(-(repmat(test_x(j,:), NumFuzzyRule,1) - center).^2/std);
    MF = prod(MF,2);  % equation 8
    MF = MF/sum(MF);  % equation 9
    F(j,:) = MF'.*(test_x(j,:)*Alpha);   % equation 11 & 12 and train_x(j,:)*b1 - equation 7
    Omega(j,:) = MF;
end
